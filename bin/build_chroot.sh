...
#############################################################################
# 
# Purpose: Creating GeoNode-Live as an Ubuntu customization
...
if [ "$#" -lt 2 ] || [ "$#" -gt 4 ]; then
    ...
    echo "Usage: build_chroot.sh ARCH(i386 or amd64) MODE(release or nightly) [git_branch (default=master)] [github_username (default=terranodo) or git clone url]"
    exit 1
fi
...
if echo "$GIT_USER" | grep -q "://"; then
    ...
    echo "Git repository: https://github.com/$GIT_USER/geonode-live.git"
fi
echo "Git branch: $GIT_BRANCH"

DIR="/usr/local/share/geonode-live/bin"
GIT_DIR="/usr/local/share/geonode-live"
VERSION=`cat "$DIR"/../VERSION.txt`
PACKAGE_NAME="geonode-live"
cd "$GIT_DIR"
...
cp "$GIT_DIR"/conf/chroot/casper.conf etc/casper.conf
...
#cp "$GIT_DIR"/conf/desktop/osgeo-desktop.png \
...
cp "$GIT_DIR/conf/chroot/README.diskdefines" extract-cd/README.diskdefines
